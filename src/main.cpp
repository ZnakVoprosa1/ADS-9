...
int main() {
  ...
  for (int n : ns) {
    ...
    tAll.push_back(duration<double>(e1 - s1).count() / 100);

    int randIdx = d(gen);
    auto s2 = high_resolution_clock::now();
    for (int i = 0; i < 100; ++i) getPerm1(t, randIdx);
    auto e2 = high_resolution_clock::now();
    t1.push_back(duration<double>(e2 - s2).count() / 100);

    auto s3 = high_resolution_clock::now();
    for (int i = 0; i < 100; ++i) getPerm2(t, randIdx);
    auto e3 = high_resolution_clock::now();
    t2.push_back(duration<double>(e3 - s3).count() / 100);
  }
  ...
  std::cout << "The data for the graph is saved in result/times.txt" << std::endl;
